...
function pickReferences(message: string, knowledgeBase: KnowledgeBase) {
  ...
  if (query.includes("api") || query.includes("endpoint")) {
    ...
    addIfPresent((ref) => ref.label.toLowerCase().includes("content sheet api"));
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      checks: {
        prompts: !!aiPrompts,
        knowledgeBase:
          knowledgeBase.sections.length > 0 || knowledgeBase.blogPosts.length > 0,
        apiKey: !!process.env.OPENAI_API_KEY,
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: ensureReferenceBlock(aiResponse, normalizedMessage, knowledgeBase),
